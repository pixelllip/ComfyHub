 ...
 *
 * 这里只登记文本 / 图片两种输入模态与工具、推理两个开关：
 * 视频、音频首期适配器根本不支持，登记了也只会被正确阻断，没有意义。
 */
object ModelCapabilityCatalog {
    ...
    private val rules: List<Triple<String, Capability, Boolean>> = buildList {
        fun rule(pattern: String, modalities: List<String>, tools: Boolean, reasoning: Boolean = false, contains: Boolean = true) {
            add(
                Triple(
                    pattern,
        ...
        rule("o3", TEXT_IMAGE, tools = true, reasoning = true)
        rule("o4-mini", TEXT_IMAGE, tools = true, reasoning = true)
        ...
        rule("o1-mini", TEXT_ONLY, tools = true, reasoning = true)
        rule("deepseek-reasoner", TEXT_ONLY, tools = true, reasoning = true)
        rule("deepseek-chat", TEXT_ONLY, tools = true)
        rule("deepseek-coder", TEXT_ONLY, tools = false)
        rule("qwq", TEXT_ONLY, tools = true, reasoning = true)
        ...
        rule("o1", TEXT_ONLY, tools = true, reasoning = true)
        rule("qwen", TEXT_ONLY, tools = true)
        rule("glm-4", TEXT_ONLY, tools = true)
        rule("glm-3", TEXT_ONLY, tools = true)
