# Krea 2 Image Reference（Krea 官方 skill 原文）

> 来源：<https://github.com/krea-ai/skills> → `krea-generate/references/models/krea-2.md`
> 许可：MIT（Copyright (c) 2026 Krea AI）
> 采集日期：2026-09（内容与当时 `main` 分支一致，逐字未改）
> 说明：这是 Krea 官方 Agent Skill 套件里**专写 Krea 2 的那一份**。
> 它面向"连着 Krea MCP 工具"的环境（`list_models` / 生成工具），
> 本机是 ComfyUI 路线，所以 `SKILL.md` 正文做了适配；本文件保留原文供核对。
