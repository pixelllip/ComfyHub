...
| AIH-008 连接测试 | ⬜ | 未做（需要 HTTP 客户端与错误分类，随 M2 适配器一起做） |
...
| AIH-027/028 严格识别 + 能力矩阵 | ◐ | 能力矩阵（模型 ∩ 适配器 ∩ MIME ∩ 大小/数量）已实现并有单测；magic bytes 分类器待做 |
...
2. **AH-027 严格分类器**：按 magic bytes 判定 image/video/audio/document，未知即 UNKNOWN 并阻断
   （**不要**复用 `MediaFiles.kindOf()` 的"未知即 IMAGE"回退）。
