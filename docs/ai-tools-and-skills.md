...
我们的工具清单（出厂 10 个，全部可在设置里改成 `allow / ask / deny`）：
...
```
<项目根>\skills\builtin\<name>\SKILL.md   内置，只读（优先级 100）
...
`%USERPROFILE%\.dsh\skills` 只在用户点「从 DSH 导入」时**读一次**并复制进来（UI 打这个动作）；
**运行期任何代码都不读 `.dsh`** —— 没装 DSH 的机器拿到的是同样的功能（这是用户明确要求的）。
...
- `name` 必须是 kebab-case（`^[a-z0-9][a-z0-9-]{0,63}$`）且与目录名一致；`description` 必填（≤600 字）；
  正文非空（≤256KB）。
...
- 导入时先校验再复制：拒绝符号链接与 `..`，限制 300 个文件 / 8MB 总量，超限整体中止。
- 注入防线写在系统提示第 4 条：工具输出与 Skill 正文都是**数据**，
...
| POST | `/api/ai/skills/import-dsh` | 从 `%USERPROFILE%\.dsh\skills` 导入 |
...
2. **第三方 Skill 的 ZIP 导入 + 预览确认**（AIH-043/044）：现在只有目录导入（DSH 那种）。
3. **内置 Anima / H3 Skills 正文**（AIH-041/042）：目前内置根是空目录，用户可以从 DSH 导入自带的 16 个。
