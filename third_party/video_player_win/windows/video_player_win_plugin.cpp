// ---------------------------------------------------------------------------
//  本文件是 video_player_win 3.2.2 的本地副本（third_party/video_player_win），
//  只打了一个补丁：initTexture() 里补上 visible_width / visible_height，
//  修 Windows 上「视频只有声音、画面全黑」的问题（Impeller 外部纹理尺寸 0x0）。
//  同步上游时请保留这个补丁，搜索 "ComfyHub 补丁" 就能找到改动点。
// ---------------------------------------------------------------------------
...
private:
  ...
  void initTexture(ID3D11Texture2D* texture)
  {
    if (!mTextureInited)
    {
      ...
      // ComfyHub 补丁（上游 3.2.2 缺失）：引擎用 visible_width / visible_height 作为
      // 外部纹理的实际尺寸。Impeller（Windows 上现在默认的渲染后端）直接拿它去
      // impeller::ISize 建纹理，少了这两个字段就是一张 0x0 的纹理 —— 现象是
      // **只有声音、画面全黑**。Skia 后端因为「0 就退回用控件尺寸」而侥幸正常，
      // 所以这个坑只在 Impeller 下暴露。详见 lib/widgets/win_video_view.dart 顶部说明。
